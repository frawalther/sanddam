...
mini_list <- list()
for (i in minikin_file[1:4]) {
  print(read_csv2(i))
  print(str_split(i, "/", simplify = T)[,12])
  mini_list[[i]] <- read_csv2(i) %>% 
    mutate(sensor = str_split(i, "/", simplify = T)[,12])
}
...
#Stack or Brick Rasters 
b <- brick(EVI_rasters)
EVI_rasters <- raster::stack(EVI_list, quick=F) 
EVI_rasters <- brick(xxx)
