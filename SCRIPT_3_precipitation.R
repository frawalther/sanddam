...
ggplot(df_prec_m, aes(x=date, y=Precip_mean, colour=X)) + 
...
read.csv("~/01Master/MasterThesis/Pius/R/sand dam/df_prec_m.csv", header=T)
...
ggplot(df_prec_d, aes(x=date, y=Precip_mean, colour=X)) + 
  geom_errorbar(aes(ymin=Precip_mean-Precip_sd, ymax=Precip_mean+Precip_sd), width=.1) +
  geom_line() +
  geom_point()
